...
def clean_column_names(schema, query_toks):
    ...
    return list(map(lambda column_name: _clean_column_name(
        column_name, column_names, column_names_original_lower, alias_to_table, tables_used), query_toks))
...
def _clean_column_name(
        candidate_column_name, column_names, column_names_original_lower, alias_to_table_map, used_table_names):
    ...
    if real_column_name.lower() in column_names_original_lower:
        ...
        if table_name is None:
            # TODO[disambiguate between table names if multiple)
            table_name = ','.join(used_table_names)
